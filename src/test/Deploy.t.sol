...
contract DeployTest is TestUtils {
    ...
    function setUp() public {
        ...
        DysonPair(pair).setFarm(address(farm));
    ...
    function testDepositWithAgent() public {
        ...
        DysonPair(pair).deposit0(alice, 1e18, 900e18, 1 days);
        assertEq(DysonPair(pair).noteCount(alice), 1);
    ...
    function testDepositWithoutAgent() public {
        vm.startPrank(bob);
        IERC20(token0).approve(pair, type(uint).max);
        DysonPair(pair).deposit0(bob, 1e18, 900e18, 1 days);
        assertEq(DysonPair(pair).noteCount(bob), 1);
