...
import "src/interfaces/IDyson.sol";
...
contract FarmTest is TestUtils {
    ...
    address dysonPair = address(0x1000);
    ...
    function testSetPool() public {
        ...
        farm.setPool(dysonPair, gauge);
        (uint _weight, uint _rewardRate, uint _lastUpdateTime, uint _lastReserve, address _gauge) = farm.pools(dysonPair);
        ...
        // Accrue reserve
        skip(1 weeks);
        uint reserve = farm.getCurrentPoolReserve(dysonPair);
    ...
    function testCannotSetPoolRewardRateByNonPoolGauge() public {
        ...
        farm.setPoolRewardRate(dysonPair, DEFAULT_REWARD_RATE, DEFAULT_POOL_WEIGHT);
    }
    ...
    function testSetPoolRewardRate() public {
        ...
        vm.prank(gauge);
        farm.setPoolRewardRate(dysonPair, DEFAULT_REWARD_RATE, DEFAULT_POOL_WEIGHT);
        (uint _weight, uint _rewardRate, , , ) = farm.pools(dysonPair);
        assertEq(_weight, DEFAULT_POOL_WEIGHT);
        assertEq(_rewardRate, DEFAULT_REWARD_RATE);
    }
    ...
    function testGrantAPWithNoBonus() public {
        _setupPool();
        ...
        uint reserveBefore = farm.getCurrentPoolReserve(dysonPair);
        ...
        emit GrantAP(alice, dysonPair, amount, expectedAPAmount);

        _grantAP(alice, amount);

        uint reserveAfter = farm.getCurrentPoolReserve(dysonPair);
        uint aliceBalanceAfter = farm.balanceOf(alice);
        (, , uint _lastUpdateTime, ,) = farm.pools(dysonPair);
    ...
    function testGrantAPWithBonus() public {
        _setupPool();
        agency.setWhois(alice, DEFAULT_AGENT_ID);
        ...
        vm.expectEmit(true, true, false, true);
        emit GrantAP(alice, dysonPair, _caluAmountWithBonus(amount, DEFAUL_BONUS), expectedAPAmount);
    ...
    function testGrantAPByNonPool() public {
        _setupPool();
        ...
        vm.expectRevert();
        uint amount = 100 * 1e18;
        farm.grantAP(alice, amount);
    }

    function testGrantAPNoopIfUserNotAgent() public {
        _setupPool();
    ...
    function _grantAP(address _user, uint _amount) internal {
        vm.prank(dysonPair);
        farm.grantAP(_user, _amount);
    }
}